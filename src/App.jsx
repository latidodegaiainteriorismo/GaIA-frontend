...
export default function App() {
  ...
  const startListening = useCallback(() => {
    ...
    const rec = new SR();
    rec.lang           = 'es-ES';
    rec.continuous     = true;   // El navegador NO para solo en pausas cortas
    ...
    let final          = '';
    let lastFinalCount = 0;   // cuántos resultados ya procesamos como finales
    let silenceTimer   = null;

    // Resetea el contador de silencio con cada detección de voz
    const resetSilenceTimer = () => {
      if (silenceTimer) clearTimeout(silenceTimer);
      silenceTimer = setTimeout(() => {
        rec.stop(); // 15s de silencio → para y envía
      }, 15000);
    };

    resetSilenceTimer(); // Empieza a contar desde el primer momento

    rec.onresult = e => {
      // Solo procesar resultados nuevos que aún no están en `final`
      ...
      // Reconstruir interim desde el último final en adelante
      let interim = '';
      for (let i = lastFinalCount; i < e.results.length; i++) {
        if (!e.results[i].isFinal) interim += e.results[i][0].transcript;
      }
      setStatusText((final + interim).trim() || 'Te escucho...');
      resetSilenceTimer(); // Cada vez que hay voz, resetea los 15s
    };
