...
export default function App() {
  ...
  const startListening = useCallback(() => {
    ...
    // --- SOLO DIAGNÓSTICO TEMPORAL: contador de reinicios + tiempo entre ellos ---
    let restartCount = 0;
    ...
    rec.onresult = e => {
      // Cada evento trae el snapshot completo reconocido hasta ahora
      // en esta sesión. Lo SUSTITUIMOS, nunca lo sumamos, para no duplicar.
      let snapshot = '';
      for (let i = 0; i < e.results.length; i++) {
        snapshot += e.results[i][0].transcript;
      }
      finalTranscriptRef.current = snapshot.trim();
      // --- SOLO DIAGNÓSTICO: prefijo [rN] con nº de reinicios actual ---
      setStatusText(`[r${restartCount}] ${finalTranscriptRef.current || 'Te escucho...'}`);
      // --- FIN BLOQUE DIAGNÓSTICO ---
    };

    rec.onerror = e => {
      // --- SOLO DIAGNÓSTICO: mostramos qué error reporta, no lo escondemos ---
      console.log('[mic diag] error:', e.error, '| transcript actual:', finalTranscriptRef.current);
      // --- FIN BLOQUE DIAGNÓSTICO ---
      ...
      // otros errores (p.ej. 'no-speech') los deja pasar: onend decide qué hacer
    };

    rec.onend = () => {
      // --- SOLO DIAGNÓSTICO: medir cada cuánto se reinicia ---
      const now = Date.now();
      const gapMs = now - lastEventTs;
      lastEventTs = now;
      console.log(`[mic diag] onend nº${restartCount + 1} | ${gapMs}ms desde el último | transcript: "${finalTranscriptRef.current}"`);
      // --- FIN BLOQUE DIAGNÓSTICO ---
      if (isRecordingRef.current) {
        // El navegador cortó la sesión solo, pero el usuario sigue grabando.
        restartCount++; // --- SOLO DIAGNÓSTICO ---
        try { rec.start(); }
