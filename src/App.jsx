...
export default function App() {
  ...
  if (!isAuthed) return (
    <div style={{ height: '100vh', background: '#E8DFF0', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Inter',-apple-system,sans-serif" }}>
  ...
  return (
    <div style={{ height: '100vh', background: '#E8DFF0', display: 'flex', fontFamily: "'Inter',-apple-system,sans-serif", color: '#4A4A46', overflow: 'hidden' }}>
      <style>{`
      ...
      {/* Sidebar */}
      <div style={{ position: 'fixed', left: 0, top: 0, bottom: 0, width: '260px', background: '#D9CEEA', borderRight: '0.5px solid rgba(74,74,70,.1)', display: 'flex', flexDirection: 'column', zIndex: 11, transform: showSidebar ? 'translateX(0)' : 'translateX(-100%)', transition: 'transform .25s ease' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '12px 16px 28px', borderTop: '0.5px solid rgba(74,74,70,.08)', flexShrink: 0, background: '#E8DFF0' }}>
          {mode === 'voice' ? (
      ...
      {showAstro && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(74,74,70,.35)', zIndex: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }} onClick={() => { setShowAstro(false); setChartSvg(null); }}>
          <div style={{ background: '#E8DFF0', borderRadius: '20px', maxWidth: '640px', width: '100%', maxHeight: '85vh', overflowY: 'auto', padding: '28px 24px', border: '0.5px solid rgba(107,158,160,.25)', boxShadow: '0 12px 40px rgba(74,74,70,.18)' }} onClick={e => e.stopPropagation()}>
