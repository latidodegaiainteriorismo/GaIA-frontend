...
export default function App() {
  ...
  const startListening = useCallback(() => {
    ...
    // iOS Safari envía resultados ACUMULATIVOS: result[N] contiene result[N-1] + más.
    // Solución: array que descarta cualquier segmento anterior que sea prefijo del nuevo.
    let transcripts  = [];
    ...
    rec.onresult = e => {
      ...
      if (result.isFinal) {
        ...
        if (newText) {
          // Eliminar segmentos anteriores que sean prefijo del nuevo (iOS acumulativo)
          // o duplicados exactos. En Chrome (frases independientes) se conservan todos.
  ...
  return (
    <div style={{ height: '100vh', background: '#FAFAF8', display: 'flex', fontFamily: "'Inter',-apple-system,sans-serif", color: '#4A4A46', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        {/* Panel de debug — visible solo cuando showDebug=true */}
        {showDebug && (
          <div style={{ maxHeight: '200px', overflowY: 'auto', background: '#1a1a2e', padding: '8px 12px', fontFamily: 'monospace', fontSize: '10px', color: '#7fdbca', borderTop: '1px solid #333', flexShrink: 0 }}>
            {debugLogs.length === 0
              ...
              : debugLogs.map((l, i) => (
                  <div key={i} style={{ color: l.includes('✅') ? '#2ecc71' : l.includes('ONERROR') ? '#e74c3c' : l.includes('ONEND') ? '#f39c12' : l.includes('START') ? '#3498db' : '#7fdbca', marginBottom: '1px', wordBreak: 'break-all' }}>{l}</div>
                ))
            }
          </div>
        )}
