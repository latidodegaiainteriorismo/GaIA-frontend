...
export default function App() {
  ...
  // Bandera para distinguir "el usuario pulsó detener" de "el navegador cortó solo"
  const isRecordingRef = useRef(false);
  // Texto reconocido en la sesión de grabación actual (snapshot completo, no acumulado a mano)
  ...
  useEffect(() => {
    ...
    const init = () => {
      ...
      window.google.accounts.id.renderButton(googleBtnRef.current, {
        ...
        locale: 'es', width: 280, shape: 'pill'
      });
    };
  ...
  const fetchConvs = useCallback(async (tok = token) => {
    if (!tok) return;
    try {
      const data = await api('/conversations', {}, tok);
  ...
  const newConv = () => {
    setCurrentConvId(null); setMessages([]);
    setTemporary(false); tempHistRef.current = [];
  ...
  // ── Audio (fix: Uint8Array, no Buffer) ────────────────────────────────────
  ...
  // ── Mic: toggle manual — pulsar para empezar, pulsar para detener y enviar ─
  const startListening = useCallback(() => {
    const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
    if (!SR) { setMode('text'); return; }

    isRecordingRef.current = true;
    // Snapshot del texto reconocido HASTA AHORA en la sesión actual de `rec`
    // (no se acumula a mano: cada evento onresult ya trae todo lo dicho
    // desde que esa instancia de rec arrancó).
    let sessionSnapshot = '';
    // Texto acumulado de sesiones anteriores, si el navegador reinició
    // el reconocimiento solo por un timeout interno mientras seguías hablando.
    let priorSessionsText = '';

    const createRecognition = () => {
      const rec = new SR();
      rec.lang = 'es-ES';
      rec.continuous = true;       // no cortar por pausas
      rec.interimResults = true;

      rec.onresult = e => {
        // Reconstruimos el snapshot completo de ESTA sesión de reconocimiento
        // a partir de TODOS los resultados que el navegador reporta —
        // sin sumar manualmente trozo a trozo, evitando duplicados.
        let snapshot = '';
        for (let i = 0; i < e.results.length; i++) {
          snapshot += e.results[i][0].transcript;
        }
        sessionSnapshot = snapshot;
        finalTranscriptRef.current = (priorSessionsText + ' ' + sessionSnapshot).trim();
        setStatusText(finalTranscriptRef.current || 'Te escucho...');
      };

      rec.onerror = e => {
        // 'no-speech' ocurre solo por silencio prolongado; no es un error real
        // mientras el usuario siga queriendo grabar, dejamos que onend reinicie.
      ...
      rec.onend = () => {
        if (isRecordingRef.current) {
          ...
          // Guardamos lo dicho hasta ahora como "sesiones previas" y
          // empezamos un snapshot nuevo en limpio para la siguiente instancia.
          priorSessionsText = (priorSessionsText + ' ' + sessionSnapshot).trim();
          sessionSnapshot = '';
          ...
          // El usuario pulsó "detener": ahora sí, enviamos lo grabado.
  ...
  const stopListening = useCallback(() => {
    isRecordingRef.current = false; // evita que onend reinicie
